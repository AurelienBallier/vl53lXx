...
bool VL53L1X::init(bool io_2v8)
{
  ...
  if (readReg16Bit(VL53L1X_DEFINITIONS::IDENTIFICATION__MODEL_ID) != 0xEACC) { return false; }
...
void VL53L1X::writeReg32Bit(uint16_t reg, uint32_t value)
{
  ...
  buffer[0] = value;
  buffer[1] = value >> 16;
  this->i2c->writeWords(reg, 2, buffer);
}
...
bool VL53L1X::setDistanceMode(VL53L1X_DEFINITIONS::DistanceMode mode)
{
  ...
  switch (mode)
  {
    case VL53L1X_DEFINITIONS::Short:
      // from VL53L1_preset_mode_standard_ranging_short_range()

      // timing config
      writeReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VCSEL_PERIOD_A, 0x07);
      writeReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VCSEL_PERIOD_B, 0x05);
      writeReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VALID_PHASE_HIGH, 0x38);

      // dynamic config
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__WOI_SD0, 0x07);
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__WOI_SD1, 0x05);
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__INITIAL_PHASE_SD0, 6); // tuning parm default
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__INITIAL_PHASE_SD1, 6); // tuning parm default

      break;

    case VL53L1X_DEFINITIONS::Medium:
      // from VL53L1_preset_mode_standard_ranging()

      // timing config
      writeReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VCSEL_PERIOD_A, 0x0B);
      writeReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VCSEL_PERIOD_B, 0x09);
      writeReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VALID_PHASE_HIGH, 0x78);

      // dynamic config
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__WOI_SD0, 0x0B);
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__WOI_SD1, 0x09);
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__INITIAL_PHASE_SD0, 10); // tuning parm default
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__INITIAL_PHASE_SD1, 10); // tuning parm default

      break;

    case VL53L1X_DEFINITIONS::Long: // long
      // from VL53L1_preset_mode_standard_ranging_long_range()

      // timing config
      writeReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VCSEL_PERIOD_A, 0x0F);
      writeReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VCSEL_PERIOD_B, 0x0D);
      writeReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VALID_PHASE_HIGH, 0xB8);

      // dynamic config
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__WOI_SD0, 0x0F);
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__WOI_SD1, 0x0D);
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__INITIAL_PHASE_SD0, 14); // tuning parm default
      writeReg(VL53L1X_DEFINITIONS::SD_CONFIG__INITIAL_PHASE_SD1, 14); // tuning parm default
...
bool VL53L1X::setMeasurementTimingBudget(uint32_t budget_us)
{
  ...
  macro_period_us = calcMacroPeriod(readReg(VL53L1X_DEFINITIONS::RANGE_CONFIG__VCSEL_PERIOD_A));
  ...
  writeReg(VL53L1X_DEFINITIONS::PHASECAL_CONFIG__TIMEOUT_MACROP, phasecal_timeout_mclks);
  ...
  writeReg16Bit(VL53L1X_DEFINITIONS::MM_CONFIG__TIMEOUT_MACROP_A, encodeTimeout(
    timeoutMicrosecondsToMclks(1, macro_period_us)));

  // "Update Range Timing A timeout"
  writeReg16Bit(VL53L1X_DEFINITIONS::RANGE_CONFIG__TIMEOUT_MACROP_A, encodeTimeout(
  ...
  writeReg16Bit(VL53L1X_DEFINITIONS::RANGE_CONFIG__TIMEOUT_MACROP_B, encodeTimeout(
    timeoutMicrosecondsToMclks(range_config_timeout_us, macro_period_us)));
...
void VL53L1X::startContinuous(uint32_t period_ms)
{
  ...
  writeReg32Bit(VL53L1X_DEFINITIONS::SYSTEM__INTERMEASUREMENT_PERIOD, period_ms * osc_calibrate_val);

  writeReg(VL53L1X_DEFINITIONS::SYSTEM__INTERRUPT_CLEAR, 0x01); // sys_interrupt_clear_range
  writeReg(VL53L1X_DEFINITIONS::SYSTEM__MODE_START, 0x40); // mode_range__timed
}
...
{
  writeReg(VL53L1X_DEFINITIONS::SYSTEM__MODE_START, 0x80); // mode_range__abort
  ...
  if (saved_vhv_init != 0)
  {
    writeReg(VL53L1X_DEFINITIONS::VHV_CONFIG__INIT, saved_vhv_init);
  }
  if (saved_vhv_timeout != 0)
  {
     writeReg(VL53L1X_DEFINITIONS::VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND, saved_vhv_timeout);
  }

  // "remove phasecal override"
  writeReg(VL53L1X_DEFINITIONS::PHASECAL_CONFIG__OVERRIDE, 0x00);
...
uint16_t VL53L1X::readRangeSingleMillimeters(bool blocking)
{
  if (blocking)
  {
    startTimeout();
    while (!dataReady())
    {
      ...
      {
        did_timeout = true;
        ranging_data.range_status = VL53L1X_DEFINITIONS::RangeInvalid;
...
void VL53L1X::setupManualCalibration()
{
  ...
  saved_vhv_init = readReg(VL53L1X_DEFINITIONS::VHV_CONFIG__INIT);
  saved_vhv_timeout = readReg(VL53L1X_DEFINITIONS::VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND);

  // "disable VHV init"
  writeReg(VL53L1X_DEFINITIONS::VHV_CONFIG__INIT, saved_vhv_init & 0x7F);

  // "set loop bound to tuning param"
  writeReg(VL53L1X_DEFINITIONS::VHV_CONFIG__TIMEOUT_MACROP_LOOP_BOUND,
    (saved_vhv_timeout & 0x03) + (3 << 2)); // tuning parm default (LOWPOWERAUTO_VHV_LOOP_BOUND_DEFAULT)

  // "override phasecal"
  writeReg(VL53L1X_DEFINITIONS::PHASECAL_CONFIG__OVERRIDE, 0x01);
  writeReg(VL53L1X_DEFINITIONS::CAL_CONFIG__VCSEL_START, readReg(VL53L1X_DEFINITIONS::PHASECAL_RESULT__VCSEL_START));
}
...
void VL53L1X::updateDSS()
{
  ...
  if (spadCount != 0)
  {
    ...
    if (totalRatePerSpad != 0)
    {
      ...
      // "override DSS config"
      writeReg16Bit(VL53L1X_DEFINITIONS::DSS_CONFIG__MANUAL_EFFECTIVE_SPADS_SELECT, requiredSpads);
  ...
  // "We want to gracefully set a spad target, not just exit with an error"
   ...
   writeReg16Bit(VL53L1X_DEFINITIONS::DSS_CONFIG__MANUAL_EFFECTIVE_SPADS_SELECT, 0x8000);
}
...
void VL53L1X::getRangingData()
{
  ...
  switch(results.range_status)
  {
    ...
    case 3: // NOVHVVALUEFOUND
      // from SetSimpleData()
      ranging_data.range_status = VL53L1X_DEFINITIONS::HardwareFail;
      break;

    case 13: // USERROICLIP
     // from SetSimpleData()
      ranging_data.range_status = VL53L1X_DEFINITIONS::MinRangeFail;
      break;

    case 18: // GPHSTREAMCOUNT0READY
      ranging_data.range_status = VL53L1X_DEFINITIONS::SynchronizationInt;
      break;

    case 5: // RANGEPHASECHECK
      ranging_data.range_status =  VL53L1X_DEFINITIONS::OutOfBoundsFail;
      break;

    case 4: // MSRCNOTARGET
      ranging_data.range_status = VL53L1X_DEFINITIONS::SignalFail;
      break;
    ...
    case 7: // PHASECONSISTENCY
      ranging_data.range_status = VL53L1X_DEFINITIONS::WrapTargetFail;
      break;

    case 12: // RANGEIGNORETHRESHOLD
      ranging_data.range_status = VL53L1X_DEFINITIONS::XtalkSignalFail;
      break;

    case 8: // MINCLIP
      ranging_data.range_status = VL53L1X_DEFINITIONS::RangeValidMinRangeClipped;
      break;
    ...
    case 9: // RANGECOMPLETE
      ...
      {
        ranging_data.range_status = VL53L1X_DEFINITIONS::RangeValidNoWrapCheckFail;
      }
      else
      {
        ranging_data.range_status = VL53L1X_DEFINITIONS::RangeValid;
      }
      break;
